"""
Homework 6：深度学习排名预测模型
使用PyTorch构建深度学习模型预测学科排名
"""
...
class RankingPredictor(nn.Module):
    ...
    def __init__(self, num_numerical_features, num_categorical_features, hidden_sizes=[64, 32, 16]):
        ...
        self.categorical_embeddings = nn.ModuleList([
            nn.Embedding(num_embeddings=1000, embedding_dim=8)  # 假设最多1000个类别
            for _ in range(num_categorical_features)
        ])
        
        # 隐藏层
        self.hidden_layers = nn.ModuleList()
        # 输入大小 = 数值特征(64) + 类别特征(8 * num_categorical_features)
        input_size = hidden_sizes[0] + (8 * num_categorical_features)
    ...
    def forward(self, numerical_features, categorical_features):
        ...
        # 处理类别特征（为每个类别特征单独嵌入然后拼接）
        cat_embeddings = []
        for i in range(categorical_features.size(1)):
            # 对每个类别特征使用对应的嵌入层
            emb = self.categorical_embeddings[i](categorical_features[:, i])
            cat_embeddings.append(emb)
        
        # 将所有类别嵌入拼接起来
        x_categorical = torch.cat(cat_embeddings, dim=1)
...
def main():
    ...
    # 只训练前5个学科作为演示（避免运行时间过长）
    subjects_to_train = list(splits.keys())[:5]
    
    for subject in subjects_to_train:
        data_parts = splits[subject]
        ...
        try:
            ...
            model, train_losses, val_losses = train_model_for_subject_dl(
                data_parts["train"], data_parts["val"], num_epochs=30
            )
            ...
            print(f"评估结果: MSE={metrics['MSE']:.2f}, MAE={metrics['MAE']:.2f}, MAPE={metrics['MAPE']:.2f}%, R2={metrics['R2']:.3f}")
    ...
    if dl_results:
        ...
        # 评估指标分布
        plt.subplot(2, 3, 1)
        plt.hist(metrics_df['MSE'], bins=10, alpha=0.7, color='skyblue')
        plt.title('MSE Distribution')
        plt.xlabel('MSE')
        
        plt.subplot(2, 3, 2)
        plt.hist(metrics_df['MAE'], bins=10, alpha=0.7, color='lightgreen')
        plt.title('MAE Distribution')
        plt.xlabel('MAE')
        
        plt.subplot(2, 3, 3)
        plt.hist(metrics_df['MAPE'], bins=10, alpha=0.7, color='lightcoral')
        plt.title('MAPE Distribution')
        plt.xlabel('MAPE (%)')
        
        plt.subplot(2, 3, 4)
        plt.hist(metrics_df['R2'], bins=10, alpha=0.7, color='gold')
        ...
        if len(dl_results) > 0:
            ...
            plt.title(f'Training Curve ({example_subject})')
            plt.xlabel('Epoch')
            plt.ylabel('Loss')
            plt.legend()
        ...
        if len(dl_results) > 0:
            ...
            plt.title(f'Predictions vs True ({example_subject})')
        
        plt.tight_layout()
        plt.savefig('results/deep_learning_results.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    print("\n深度学习排名预测完成！")
    print("主要评估指标说明:")
    print("- MSE (均方误差): 值越小越好，惩罚大误差")
    print("- MAE (平均绝对误差): 值越小越好，直观反映平均偏差")
    print("- MAPE (平均绝对百分比误差): 值越小越好，相对误差度量") 
    print("- R2 (决定系数): 越接近1越好，解释方差比例")
